...
class SNP_NHP_Options_custom_fields extends SNP_NHP_Options
{
	...
	elseif(is_array($atts) && isset($atts['type']) && $atts['type']=='name')
	{
	   ?>
	...
	?>
	<div class="snp-cf-field <?php if($tpl==1) {echo 'snp-cf-field-tpl';} ?>">
		...
		<?php /*Label: <span class="snp-label"><?php echo $atts['label']; ?></span><br />
	...
	<script>
		...
		cf.find('.snp-cf-add-option').click(function(){
		    //$(this).before($(this).prev('input').clone().val(''));
