...
{
	echo '.snp-pop-'.$ID.' .snp-theme15 { width: '.$POPUP_META['snp_width'].'px;}'."\n";
}
if (!empty($POPUP_META['snp_height']))
{
	echo '.snp-pop-'.$ID.' .snp-theme15 { min-height: '.$POPUP_META['snp_height'].'px;}'."\n";
}
if (!empty($POPUP_META['snp_header_font']['size']))
{
	echo '.snp-pop-'.$ID.' .snp-theme15 h2 {font-size: '.$POPUP_META['snp_header_font']['size'].'px; color: '.$POPUP_META['snp_header_font']['color'].';}'."\n";
}
if (!empty($POPUP_META['snp_subheader_font']['size']))
{
	echo '.snp-pop-'.$ID.' .snp-theme15 h3, .snp-pop-'.$ID.' .snp-theme15 .snp-newsletter-content p {font-size: '.$POPUP_META['snp_subheader_font']['size'].'px;}'."\n";
	echo '.snp-pop-'.$ID.' .snp-theme15 h3, .snp-pop-'.$ID.' .snp-theme15 .snp-newsletter-content p {color: '.$POPUP_META['snp_subheader_font']['color'].';}'."\n";
}
if (!empty($POPUP_META['snp_submit_button_text_color']))
{
    	echo '.snp-pop-'.$ID.' .snp-theme15 .snp-submit { color: '.$POPUP_META['snp_submit_button_text_color'].';}'."\n";
}
if (!empty($POPUP_META['snp_submit_button_color']))
{
    	echo '.snp-pop-'.$ID.' .snp-theme15 .snp-submit { background-color: '.$POPUP_META['snp_submit_button_color'].';}'."\n";
	echo '.snp-pop-'.$ID.' .snp-theme15 form input[type="text"],.snp-pop-'.$ID.' .snp-theme15 form select,.snp-pop-'.$ID.' .snp-theme15 form textarea,.snp-pop-'.$ID.' .snp-theme15 form input[type="text"]:focus,.snp-pop-'.$ID.' .snp-theme15 form select:focus,.snp-pop-'.$ID.' .snp-theme15 form textarea:focus {border: 1px solid '.$POPUP_META['snp_submit_button_color'].';}';
}
if (!empty($POPUP_META['snp_submit_button_hover_color']))
{
    	echo '.snp-pop-'.$ID.' .snp-theme15 form input[type="submit"]:hover { background-color: '.$POPUP_META['snp_submit_button_hover_color'].';}'."\n";
}

if (!empty($POPUP_META['snp_bg_color']))
{
    	echo '.snp-pop-'.$ID.' .snp-theme15 .snp-submit-content { background-color: '.$POPUP_META['snp_bg_color'].';}'."\n";
}
if (!empty($POPUP_META['snp_bg_img']))
{
    	echo '.snp-pop-'.$ID.' .snp-theme15 { background: url(\''.$POPUP_META['snp_bg_img'].'\') no-repeat scroll 50% 50% transparent; }'."\n";
}
